...
* **Embedded / IoT**: Focused on strict memory management and hardware interactions (C, C++, bare-metal Rust). Specialized in microcontrollers, RTOS, and highly constrained computational environments.
* **Systems Scripting**: Aimed at low-level OS tasks, bash/powershell scripting, automation, system performance tuning, and CLI tool development.
...
* **Pre-Commit README Badges Verification:** Before staging and executing a `git commit` (or cutting a release), you MUST inspect the project's root `README.md`. If it does NOT already contain an `img.shields.io` badge block below the primary `# [Project Title]` header, you MUST pause and proactively prompt the user with a tailored list of suggested `img.shields.io` badges (e.g., build status, release version, license, tech stack, and repo size) derived from `project_details/github.md`. If the user approves or selects badges, format and insert them into `README.md` before committing. If the user declines, proceed without them.
* **Environment-Centric Configuration (.env / .env.example):** Project credentials, deployment targets, and service identifiers (such as GitHub remote URLs, Vercel org/project IDs, Firebase project IDs, API keys, and auth secrets) must NEVER be hardcoded, tracked, or committed inside `project_details/*.md` artifacts or documentation files. All service configuration MUST live strictly in `.env`, with corresponding documented templates in `.env.example`. Agents must read configuration directly from `.env`, self-healing missing keys using `.env.example` as a template.
...
* **CRITICAL - PROJECT ROOT CONTAINMENT:** NEVER escape, read from, write to, or execute commands in parent or external directories outside the project root directory unless there is explicit reasoning provided with a direct request for human approval. All file modifications, directory traversals, and command executions must remain strictly confined within the project root boundary.
